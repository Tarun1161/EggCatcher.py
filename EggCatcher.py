...
conn = mysql.connector.connect(
    ...
    password="1161",# Your MySQL password
    database="eggcatcherdb"
)
cursor = conn.cursor()
...
def show_highscores():
    ...
    leaderboard = "\n".join([f"{row[0]} - {row[1]} pts" for row in results])
    return leaderboard

# ----------------- Game Over -----------------
def game_over():
    global score
    player_name = simpledialog.askstring("Game Over", f"Your Score: {score}\nEnter your name:")
    if player_name:
        save_score(player_name, score)
    leaderboard = show_highscores()
    messagebox.showinfo("Leaderboard", f"🏆 High Scores:\n\n{leaderboard}")
    cursor.close()
    conn.close()
    root.destroy()
